...
void CCOllisionMgr::Collision_Bullet_Wall(list<BulletInfo*>* pObjs, vector<CObj*>* pWalls)
{
	for (auto& pObj : *pObjs)
	{
		float fX = pObj->fX;
		...
		int* p = CTileMgr::Get_Instance()->Get_CullSize(pObj->fX, pObj->fY, 2);
		RECT temp = {};
		for (int i = p[1]; i < p[3]; ++i)
		{
			for (int j = p[0]; j < p[2]; ++j)
			{
				...
				if (0 > iIndex || iIndex > 8000 || static_cast<CTile*>((*pWalls)[iIndex])->Get_Option() == 0)
					continue;
				RECT BulletRect = {};
				BulletRect.left = LONG(fX - iCX / 2);
				BulletRect.top = LONG(fY - iCY * 0.5f);
				BulletRect.right = LONG(fX + (iCX >> 1));
				BulletRect.bottom = LONG(fY + (iCY >> 1));
...
void CCOllisionMgr::Collision_Object_Wall(PlayerInfo* pPlayerInfo, vector<CObj*>* pWalls)
{
	...
	int* p = CTileMgr::Get_Instance()->Get_CullSize(fX, fY, 3);
	RECT rc = { p[0] * TILECX, p[1] * TILECY, p[2] * TILECX, p[3] * TILECY };
	POINT pt = { (LONG)(fX), (LONG)(fY) };
	if (!PtInRect(&rc, pt))
		return;
	RECT temp = {};
	for (int i = p[1]; i < p[3]; ++i)
	{
		for (int j = p[0]; j < p[2]; ++j)
		{
			...
			if (0 > iIndex || iIndex > pWalls->size() || static_cast<CTile*>((*pWalls)[iIndex])->Get_Option() == 0)
				continue;
			...
			if (IntersectRect(&temp, &PlayerRect, (*pWalls)[iIndex]->Get_Rect()))
			{
				float fXValue = (float)(temp.right - temp.left);
				float fYValue = (float)(temp.bottom - temp.top);
				if (fXValue > fYValue)
				{
					if (fY < (*pWalls)[iIndex]->Get_Info()->fY)
					{
						pPlayerInfo->fY -= fYValue;
						//pObj->Move_Pos(0, -fYValue);
					}
					else
						pPlayerInfo->fY += fYValue;
						//pObj->Move_Pos(0, fYValue);
				...
				{
					if (fX < (*pWalls)[iIndex]->Get_Info()->fX)
					{
						pPlayerInfo->fX -= fXValue;
						//pObj->Move_Pos(-fXValue, 0);
					}
					else
						pPlayerInfo->fX += fXValue;
						//pObj->Move_Pos(fXValue, 0);
